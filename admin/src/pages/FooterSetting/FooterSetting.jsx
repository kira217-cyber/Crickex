...
const emptyFooter = {
  ...
  description: {
    ...
    en: "With numerous sports betting options, including cricket, football, kabaddi, basketball, tennis, and many others, Crickex is at the forefront as one of the best online sports betting sites and casinos in Bangladesh.",
  },
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Control footer payment methods, socials, sponsors, ambassadors,
              links, logo and description from admin panel.
            </p>
          </div>
            ...
            {form.socials?.map((item, index) => (
              <Card key={item._id || item.localId || index}>
                ...
                <SmallInput
                  label="Link"
                  value={item.link}
                  onChange={(v) => updateArrayItem("socials", index, "link", v)}
            ...
            {form.sponsors?.map((item, index) => (
              <Card key={item._id || item.localId || index}>
                <ImageInput
                  ...
                  onChange={(file) => updateArrayImage("sponsors", index, file)}
                />
                ...
                <LocalizedInput
                  ...
                  value={item.name}
                  onChange={(lang, value) =>
                    updateArrayLocalized("sponsors", index, "name", lang, value)
                  }
                />
                ...
                <LocalizedInput
                  ...
                  value={item.sub}
                  onChange={(lang, value) =>
                    updateArrayLocalized("sponsors", index, "sub", lang, value)
                  }
                />
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {form.links?.map((item, index) => (
              <Card key={item._id || item.localId || index}>
                ...
                <CardActions onRemove={() => removeArrayItem("links", index)} />
              </Card>
            ))}
          </div>
        </ListSection>

        <div className="sticky bottom-4 z-20 rounded-3xl border border-[#1A79D3]/20 bg-black/80 p-4 shadow-2xl backdrop-blur">
          <button
            type="submit"
            disabled={loading}
